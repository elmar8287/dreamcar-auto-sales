  ...
  return (
    <section className="mt-20 mx-auto px-4 max-w-screen-xl md:px-8" id="cars">
      <div className="text-center">
        <h1 className="text-3xl text-gray-800 font-semibold">Later updates</h1>
      ...
      <div className="mt-12 grid grid-cols-2 gap-2 sm:grid-cols-4">
        {carsList
          .map((car) => (
            <article
              className="w-full mt-4 shadow-lg border rounded-md duration-300 hover:shadow-sm"
              key={car.id}
            >
              <a href={car.link}>
                <img
                  ...
                  loading="lazy"
                  alt={car.model}
                  className="w-full h-38 md:h-56 rounded-t-md"
                  onError={(e) => {
                    e.target.onerror = null;
                    e.target.src =
                      "https://img.freepik.com/free-vector/sold-out-price-tag-sign_123447-162.jpg";
                  }}
                />

                <div className="pt-3 ml-4 mr-2 mb-3">
                  <h3 className="text-md text-gray-900">{car.model}</h3>
